...
async function loadConfiguration(productId) {
    try {
        const response = await fetch(`https://centers-lancaster-romance-newman.trycloudflare.com/api/public/configurator/${productId}?v=${Date.now()}`,
            {
        ...
        if (!data.success) {
            ...
            showError('Failed to load product configuration hghghgghghhggh');
            return null;
        }
...
async function calculatePrice(productId, selections, measurements, quantity) {
    try {
        const response = await fetch(`https://centers-lancaster-romance-newman.trycloudflare.com/api/public/calculate-price?v=${Date.now()}`, {
            method: 'POST',
            cache: 'no-store', // 🚫 disable cache
            headers: {
